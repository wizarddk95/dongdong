...
- **말로 설명하는 대신 보여 준다.** 스크린샷을 `Ctrl+V` 로 붙여넣거나 클립 버튼으로 골라
  실제 이미지로 보낸다. 바이트는 워크스페이스에 **내용 지문(SHA-256)** 으로 복사되므로,
  원본을 지운 뒤에 그 턴을 다시 열어도 무엇을 보냈는지가 남는다. 긴 변 1568px 을 넘으면
  먼저 줄인다 — 어차피 공급자가 줄이면서 요금은 원본 기준으로 매긴다.
  대화에 저장되는 것은 base64 가 아니라 참조 한 줄이라 컨텍스트 스냅샷이 부풀지 않고,
  컨텍스트 게이지는 이미지 토큰을 공급자별 공식으로 따로 센다.
...
- **읽는 면을 넓힌다.** 좌측 세션 목록은 `Ctrl+B`, 우측 대화 트리 패널은 `Ctrl+L` 로 접는다.
  상단 바의 토글 두 개가 같은 일을 하고, 접은 상태는 저장된다.
  채팅 입력칸도 경계선을 끌어 위아래로 늘린다(더블클릭하면 기본 높이).
...
src/                        프론트엔드 (React + TS + Tailwind v4)
  ...
  lib/panelSize.ts          좌우 패널 폭 · 입력칸 높이 클램프(순수)
...
- **카드가 대화를 가리지 않는다.** 카드는 대화 목록 위에 뜨는 팝업이 아니라 목록 **아래**에
  자리를 차지하는 형제다. 그래서 카드가 뜨면 목록이 그만큼 줄어드는데, 스크롤 위치를 그대로
  두면 방금 도착한 답이 화면 밖으로 밀려 "팝업이 새 메시지를 가렸다" 로 보인다 →
  목록 높이가 바뀌면 바닥에 붙어 있던 사람은 다시 바닥으로 데려간다(위로 올려 옛 답을
  읽는 중이면 건드리지 않는다). 카드 자체도 화면의 절반을 넘기지 못하고, 넘치면 카드
  안쪽이 스크롤한다. 승인 카드도 같다.
...
붙이는 길은 넷이다 — 입력칸에 **`Ctrl+V`** 로 붙여넣기, 입력칸 위로 **끌어다 놓기**,
오른쪽 **클립 버튼**으로 고르기, 그리고 프로젝트 안 이미지는 **`@경로`** 로 지목하기.
모델은 글이 아니라 **실제 이미지**를 받는다.

넷이 **같은 길로 흐른다** — 같은 형식·같은 장수 상한·같은 모델 게이팅을 지나 같은 자리에 눕는다.
경로만 적어 두는 두 번째 표를 만들지 않는 것이 요점이다(그러면 전송 직전 하이드레이션에
분기가 하나 더 생기고, 그 분기는 반드시 한쪽만 고쳐진다).

- **바이트는 대화에 넣지 않는다.** `.agent_workspace/attachments/<sha256>.<확장자>` 에
  내용 지문으로 눕고, 사용자 노드의 본문에는 마커 한 줄(`<image sha="…" w="…" h="…" />`)만 남는다.
  base64 를 본문에 넣으면 말풍선 · 인스펙터 · 페이로드 계산이 전부 메가바이트 문자열을 만지게 되고,
  무엇보다 **`context_snapshot` 이 스텝마다 그걸 한 벌씩 복사한다**(5스텝 턴이면 다섯 벌).
  진짜 바이트로 바뀌는 곳은 전송 직전 한 곳뿐이다(`hydrateImages()`).
- **원본을 지워도 그 턴의 기록은 남는다.** 경로만 적어 두면 스크린샷을 지우는 순간
  "그때 무엇을 보냈는가" 가 사라진다. 첨부의 대전제가 이미지에서 더 중요하다.
- **같은 이미지를 여러 번 붙여도 파일은 하나다.** 지문이 곧 파일 이름이라 중복이 생기지 않는다.
  (지문은 웹뷰가 계산하고, Rust 는 그 값이 소문자 hex 64자인지 다시 본다 — 그게 파일 이름이 되므로
  검사를 빠뜨리면 `../` 한 줄로 워크스페이스 밖에 쓰게 된다.)
- **긴 변 1568px 을 넘으면 먼저 줄인다.** 공급자는 어차피 서버에서 줄이면서 **요금은 원본
  기준**으로 매긴다 — 4K 스크린샷 한 장이 ~5,000 토큰이고 줄이면 ~1,600 토큰이다.
  줄일 때만 webp 로 다시 굽고, 이미 작으면 원본 바이트를 그대로 쓴다.
- **컨텍스트 게이지는 이미지를 따로 센다.** 게이지는 "자 수 ÷ 실측 토큰" 비율로 굴러가는데
  이미지는 페이로드에 참조 한 토막으로만 잡혀서 자 수가 거의 0이다. 그대로 두면 비율이
  망가져 남은 대화를 통째로 과소평가한다 → 이미지 몫은 환산에서 빼고 **공급자별 공식**으로
  더한다(Anthropic `넓이×높이÷750` · Google 768 타일당 258 · OpenAI 512 타일당 170+85).
  전부 근사이고, 정확한 값은 턴이 끝난 뒤 공급자가 세어 준다.
- **파일 선택은 `plugin-dialog` 이 아니라 웹뷰의 `input[type=file]` 이다.** 대화상자는
  **경로**를 주는데 바탕화면 스크린샷은 프로젝트 루트 밖이라 `resolve_within()` 이 막는다.
  붙여넣기·파일 선택은 바이트를 곧바로 주므로 루트 담장에 구멍을 낼 이유가 없다.
- 한 메시지에 최대 5장. PNG · JPEG · WebP · GIF 만 받는다(확장자는 Rust 의 허용 목록에서만 나온다).
- 이미지를 안 받는 모델을 골라 두면 클립 버튼이 잠긴다. 다만 카탈로그가 `supportsVision` 을
  **적어 두지 않은** 모델은 막지 않는다 — `MODEL_CATALOG` 은 사용자 소유라 우리가 채우지 않고,
  모른다고 잠그면 멀쩡히 이미지를 받는 모델에서 버튼이 사라진다.
- 턴을 지워도 눕은 바이트는 지우지 않는다. **되돌리기가 그 파일을 필요로 한다**
  (서브에이전트 기록을 안 지우는 것과 같은 이유다).

### `@` 로 프로젝트 안 이미지 지목하기

`@` 자동완성 목록에서 `.png` · `.jpg` · `.webp` · `.gif` 를 고르면 목록에 **"이미지 첨부"**
라고 미리 적히고, 보낼 때 텍스트가 아니라 바이트로 실린다.

- **프로젝트 안 이미지도 워크스페이스로 복사한다.** 붙여넣기와 같은 이유다 — 파일이 바뀌거나
  지워져도 그 턴의 기록은 그대로여야 한다. 덕분에 마커·참조·하이드레이션·토큰 계산이 전부
  그대로 재사용된다.
- 바이트를 가져오는 길은 `read_file_base64` 이고, `read_file` 과 **같은 담장**
  (`resolve_within()`)을 지난다. 이 command 로 읽을 수 있는 것은 `read_file` 로도 읽을 수
  있다 — 내용이 글자로 안 보일 뿐이다. 루트 밖은 여기로 들어오지 못한다(그쪽은 붙여넣기의 몫).
- **`.svg` 는 이미지로 치지 않는다.** SVG 는 XML 텍스트라 본문으로 실어 주는 편이 모델에게
  훨씬 쓸모 있다(코드 펜스에 `xml` 로 붙는다).
- 이미지를 안 받는 모델을 골라 두면 `@shot.png` 는 **자리표로 물러난다** — 전송이 400 으로
  끊기는 대신 "이 모델은 이미지를 받지 않아 싣지 않았습니다" 가 남는다.
- 상한은 **자가 둘이다**: 텍스트는 글자 수(파일당 20,000자·합계 60,000자), 이미지는 장수(5장).
  이미지 한 장은 본문에 마커 한 줄이라 글자로 재면 공짜처럼 보이는데 실제로는 토큰 수천을 먹는다.

### 끌어다 놓기

입력칸 영역 위로 파일을 끌고 오면 테두리가 청록으로 바뀌고, 놓으면 붙여넣기와 똑같이 처리된다.
폴더나 이미지가 아닌 파일이 섞여 오면 조용히 삼키지 않고 **왜 안 붙었는지**를 적어 준다.

여기서 걸리는 함정 하나: **Tauri 창의 `dragDropEnabled` 를 꺼야 한다**
(`src-tauri/tauri.conf.json`). 켜져 있으면 OS 레벨에서 창이 드래그를 먼저 가로채고
웹뷰의 `dataTransfer` 는 **빈 채로** 온다 — 윈도우에서 특히 그렇다. 우리는 `onDragDropEvent`
를 쓰는 곳이 없으므로 꺼도 잃는 것이 없다.
...
- **`[+ 파일]` · `[+ 폴더]` 로 만든다.** 지금 보고 있는 폴더에 빈 파일 / 새 폴더를 만드는
  버튼이 각각 하나씩이다. 누르면 목록 맨 위에 이름 칸이 열리고 `Enter` 로 만들고 `Esc` 로 접는다.
  만들어질 자리(`/src/lib` 같은 경로)는 **누르기 전에** 칸 아래에 적혀 있다.
- **이름은 만들기 전에 걸러 낸다.** 경로 구분자(`/` · `\`) · 윈도우가 못 쓰는 글자
  (`< > : " | ? *`) · 예약 장치 이름(`con` · `nul` · `com1` …) · **이미 있는 이름**이면
  왜 안 되는지가 칸 아래에 뜨고 [만들기] 가 잠긴다. 판정은 순수 함수(`lib/fileNames.ts`)라
  화면과 테스트가 같은 규칙을 본다. 중복은 대소문자를 접어서 본다 — 윈도우에서
  `README.md` 와 `readme.md` 는 같은 파일이라, 다른 이름인 줄 알고 만들면 덮어쓴다.
- 목록에 안 보이는 숨김 파일과 부딪히는 경우가 있어 **만들기 직전에 디스크에 한 번 더 묻는다**
  (`path_info`). `write_file` 은 말없이 덮어쓰므로 이 확인이 없으면 "새 파일" 한 번이 남의
  파일을 비운다.
- 만든 파일은 곧바로 뷰어에서 열린다 — 만든 다음에 하려던 일이 대개 편집이다.
  점으로 시작하는 이름(`.env`)을 만들면 숨김 표시도 함께 켜진다(안 그러면 만든 것이 안 보인다).
...
**양옆 패널은 접을 수 있다.** 좌측 세션 목록은 `Ctrl+B`, 우측 대화 트리 패널은 `Ctrl+L`(맥은
`Cmd`)이고 상단 바의 토글 두 개가 같은 일을 한다. 입력칸에 커서를 둔 채로도 그대로 눌린다 —
수식 키가 붙은 조합이라 타이핑과 헷갈릴 일이 없다. 접으면 분할선도 함께 사라진다(끌 패널이
없는 손잡이는 잡을 것이 없다). 폭과 달리 **접은 상태는 저장된다** — "이 패널은 안 본다" 는
결정이 앱을 다시 켤 때마다 되살아나면 접은 뜻이 없다.

**입력칸 높이도 끌어서 바꾼다.** 대화와 입력이 맞닿는 선이 그대로 손잡이다(더블클릭하면 기본
높이로). 세 줄짜리 창으로 긴 지시문을 쓰면 방금 쓴 문단이 스스로 보이지 않는다. 고른 높이는
배율과 같은 이유로 `settings.json` 에 남는다.
