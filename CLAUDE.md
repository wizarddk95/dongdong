...
- **스킬**: 절차서(`SKILL.md`)다. 시스템 프롬프트에는 `이름: 설명` 한 줄씩만 실리고, 모델이
  ...
  **설정의 [스킬] 에서 본문까지 고친다** — 이름만 만들 수 있으면 사람은 결국 바깥 편집기를
  열어야 하고, 그러면 스킬은 "설정에 있지만 설정에서 못 쓰는 것" 이 된다. 고치는 것은
  **파일 원문**(frontmatter 포함)이다: 본문만 내주면 저장할 때 머리말을 다시 조립해야 하고,
  그 조립이 파서와 어긋나는 순간 사람이 적은 `description` 이 조용히 사라진다(그 한 줄이
  매 턴 실리는 전부다). 쓰는 길은 `write_file` 이 아니라 `write_skill_file` 이다 —
  전역 스킬은 프로젝트 루트 밖이라 `resolve_within()` 을 지날 수 없고, 대신 **두 스킬
  디렉터리 안의 `.md`** 만 받는다(삭제와 같은 담장). 스킬 이름은 **한글도 받는다** —
  아스키가 아닌 글자를 전부 하이픈으로 갈면 `사내-보고서-양식`(placeholder 의 예)이
  통째로 비어 튕긴다. 접을 것은 글자 종류가 아니라 **경로가 되는 글자**다.
