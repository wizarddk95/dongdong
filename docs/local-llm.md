...
**드롭다운의 로컬 모델은 이 목록이 전부다.** 아래 추천표에 있어도 안 받아 놓은 모델은
뜨지 않는다 — 없는 태그를 고르면 호출이 404 로 죽는데 목록만 봐서는 "설치됨" 인지
"추천" 인지 구분할 수 없기 때문이다. 앱을 띄울 때도 한 번 자동으로 다시 물어보므로,
`ollama pull` 로 새로 받았다면 앱을 다시 띄우거나 [설치된 모델 불러오기] 를 누르면 된다.
(서버가 꺼져 있으면 마지막으로 확인한 목록을 그대로 둔다)
